...
public class UserDAO {
    ...
    public void insert(Player player) throws SQLException {
        String sql = "INSERT INTO users (id, username, password_hash, email, email_verified, verify_token, verify_sent_at, created_at) VALUES (?,?,?,?,0,?,?,NOW())";
        try (Connection c = DBConnection.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, player.getId());
            ps.setString(2, player.getUsername());
            ps.setString(3, player.getPasswordHash());
            ps.setString(4, player.getEmail());
            ps.setString(5, player.getVerifyToken());
    ...
    public void insertGoogleUser(Player player) throws SQLException {
        String sql = "INSERT INTO users (id, username, password_hash, email, email_verified, google_id, created_at) VALUES (?,?,NULL,?,1,?,NOW())";
        try (Connection c = DBConnection.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, player.getId());
            ps.setString(2, player.getUsername());
    ...
    public void insertFacebookUser(Player player) throws SQLException {
        String sql = "INSERT INTO users (id, username, password_hash, email, email_verified, facebook_id, created_at) VALUES (?,?,NULL,?,1,?,NOW())";
        try (Connection c = DBConnection.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, player.getId());
            ps.setString(2, player.getUsername());
            ps.setString(3, player.getEmail());
            ps.setString(4, player.getFacebookId());
            ps.executeUpdate();
        }
    }
